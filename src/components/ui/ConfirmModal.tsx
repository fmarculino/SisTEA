...
export function ConfirmModal({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        {/* Background Accent */}
        <div className={`absolute top-0 left-0 w-full h-1 ${button.split(' ')[0]}`} />
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex gap-3 w-full">
            <button
              onClick={onClose}
              className="flex-1 py-3.5 rounded-2xl border border-border bg-background hover:bg-muted text-foreground text-sm font-bold transition-all active:scale-95"
            >
              {cancelText}
            </button>
            <button
