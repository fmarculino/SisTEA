        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
          ...
          <div className="sm:col-span-1">
            ...
            {selectedProcedurePrice && selectedProcedurePrice.contract_id && (
              <div className={`mt-3 flex flex-col gap-2.5 p-3.5 rounded-2xl border transition-all animate-in fade-in slide-in-from-top-2 duration-200 ${selectedProcedurePrice.quantidade_saldo <= 0
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`shrink-0 ${selectedProcedurePrice.quantidade_saldo <= 0 ? 'text-rose-500 animate-pulse' :
                  ...
                  <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${selectedProcedurePrice.quantidade_saldo <= 0
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      console.error('CORS or fetch error for print, falling back to new tab:', error)
                      window.open(url, '_blank')
                      setIsPrinting(false)
                    }
                  }}
                  className={`inline-flex items-center justify-center p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted transition-all ${previewAttachment.isImage ? 'hidden' : ''} ${isPrinting ? 'opacity-50 cursor-not-allowed' : ''}`}
                  title={isPrinting ? "Preparando impressão..." : "Imprimir"}
                >
                  {isPrinting ? (
