...
import { createCidAction, updateCidAction } from './actions'

export function CidForm({ 
  initialData, 
  id 
}: { 
  initialData?: Partial<CidFormData>; 
  id?: string;
}) {
  ...
  const {
    ...
    resolver: zodResolver(cidSchema) as any,
    defaultValues: {
  ...
  async function onSubmit(data: CidFormData) {
    setIsPending(true)
    setErrorMsg('')
    try {
      let result
      if (id) {
        result = await updateCidAction(id, data)
      } else {
        result = await createCidAction(data)
      }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 bg-card p-8 rounded-xl border border-border shadow-xl">
      ...
      <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        <div className="sm:col-span-2">
          <label className="block text-sm font-semibold text-foreground mb-1 uppercase tracking-tight">Código CID *</label>
          <input
            type="text"
            {...register('code')}
        ...
        <div className="sm:col-span-4">
          <label className="block text-sm font-semibold text-foreground mb-1 uppercase tracking-tight">Descrição / Nome do Diagnóstico *</label>
          <input
            type="text"
            {...register('name')}
        ...
        <div className="sm:col-span-6">
          <label className="flex items-center space-x-3 group cursor-pointer">
            ...
            />
            <span className="text-sm font-bold text-foreground uppercase tracking-wider group-hover:text-primary transition-colors">CID Ativo</span>
          </label>
        </div>
      </div>
